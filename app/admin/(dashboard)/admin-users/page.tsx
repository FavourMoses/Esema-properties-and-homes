...
export default async function AdminUsersPage() {
  ...
  const users = await db.select().from(schema.adminUsers).orderBy(asc(schema.adminUsers.createdAt));

  return (
    <div>
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-display text-2xl font-bold text-[var(--color-navy)]">Admin Users</h1>
      ...
      <div className="mt-6 space-y-3">
        {users.map((u) => {
          ...
          return (
            <div key={u.id} className="rounded-lg border border-[var(--color-border)] bg-white p-4">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <p className="flex items-center gap-2 font-semibold text-[var(--color-navy)]">
                    ...
                    {isSelf ? <span className="text-xs font-normal text-[var(--color-ink-soft)]">(you)</span> : null}
                  </p>
                  <p className="text-xs text-[var(--color-ink-soft)]">{u.email}</p>
                  {u.role === "staff" ? (
                    <p className="mt-2 text-xs text-[var(--color-ink-soft)]">
                      {u.permissions.length > 0
                        ? u.permissions.map((p) => PERMISSION_LABELS[p as PermissionKey] ?? p).join(" · ")
                        : "No sections assigned yet"}
                    </p>
                  ) : (
                    <p className="mt-2 text-xs text-[var(--color-ink-soft)]">Full access to everything</p>
                  )}
                </div>

                {u.role === "owner" ? null : (
                  <div className="flex gap-4">
                    <Link href={`/admin/admin-users/${u.id}/edit`} className="text-xs font-semibold text-[var(--color-forest)] hover:underline">
                      Edit
                    </Link>
                    <form action={deleteAdminUser.bind(null, u.id)}>
                      <ConfirmSubmitButton confirmMessage={`Remove ${u.name}'s account?`} className="text-xs font-semibold text-[var(--color-danger)] hover:underline">
                        Delete
                      </ConfirmSubmitButton>
                    </form>
                  </div>
